...
unix utility `cut` to take only the firt 12 columns from a file.

```sh
cut -f1-12 [alignment_with_tags.paf] > tagless.paf
```

With not tag dat to consider, `read_paf` will be able to deal with this data more 
quickly. `pafr` also provides the functoin `as_paf` to allow users to convert a
12-column `data.frame` or simlar into a `pafr` object that can be used by
...
data into a useable form and adding layers to plots. Each of these functions 
...
The 'highlight' functions use ggplot's `geom_rect` to prodice these annotatoins,
